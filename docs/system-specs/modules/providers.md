...
`DefaultProviderRegistry.create_factory` chooses
...
resolution in a separate cached factory. The adapter subclass declares
...
admits only Kiro CLI, KAS, and Claude. Codex, goose, OpenCode, pi, and
registry-only adapters remain described and discoverable but cannot be persisted
until basic validation evidence admits them to the set. Codex's `mode=read-only`
blocks writes but does not permission-route passive reads; because the standard
sandbox leaves credential homes readable, that is not enough evidence for
admission. KAS is fully described and is selectable (cli-fronted via `kiro-cli`).
...
`AcpBackendCard` and the preview-gated Services row share the same React Query
  ...
  adapter resolvers for an invisible label.
...
`AcpProvider.start()` branches on named backend capabilities:
