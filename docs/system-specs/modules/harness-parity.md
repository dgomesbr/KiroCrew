...
|---|---|---|---|
| H1 | `agent.acp_backend` defaults to `ACP_BACKEND_KIRO`, and `ACP_BACKEND_KIRO` is in `ACP_BACKENDS_SELECTABLE` unconditionally. An operator who configures nothing, and an operator whose configuration is unusable, both get the Kiro harness. The initial preview set is pinned to Kiro CLI, KAS, and Claude; discovery cannot widen it without validation evidence and an explicit membership edit. Codex remains described but withheld because its read-only mode does not permission-route passive reads, while the standard sandbox leaves credential homes readable. | `test_harness_parity.py::test_kiro_is_the_default_backend`, `::test_kiro_is_always_selectable`, `::test_initial_adapter_selection_is_limited_to_reviewed_backends` | `config/loader.py` (`AgentConfig.acp_backend`), `acp/types.py` (`ACP_BACKENDS_SELECTABLE`) |
...
| Id | Guarantees | Pinned by | Constrains |
|---|---|---|---|
| H9 | `kiro-cli` keeps its own positive branch of spawn-argv resolution, including pre-spawn agent materialization (`kiro-cli` discovers selectable modes from `~/.kiro/agents/*.json` at startup, so a later `set_mode` fails with "Mode not found" without it) and its `--model` pin (the only way to run a model outside the agent's provider). A generic fallback or dict-of-builders refactor that treats Kiro as one entry among N either grants its machinery to a future harness or drops these steps. | `test_harness_parity.py::test_kiro_spawn_argv_keeps_its_own_branch`, `test_acp_runtime.py::test_runtime_unmapped_backend_refuses_before_kiro_resolution` | `acp/runtime.py` (`AcpRuntime._resolve_spawn_argv`) |
...
| Id | Guarantees | Pinned by | Constrains |
|---|---|---|---|
| H13 | Harness support is additive at the `ProviderRegistry` seam: a v1 addition, no `CONTRACT_VERSION` bump. The Kiro construction path is a direct `AcpProvider` factory body; it neither calls the adapter-factory helper nor gains a conditional, required argument, or failure mode in service of an adapter. | `test_provider_dispatch.py::TestAcpBackendOverride::test_direct_config_factory_isolated_from_adapter_factory`, plus review (`AUTOSDE.yaml` → `harness-parity`) | `platform/interfaces.py` (`ProviderRegistry`), `config/loader.py` (`create_provider_factory`) |
