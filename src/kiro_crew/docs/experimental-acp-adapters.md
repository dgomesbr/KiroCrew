...
Kiro CLI, KAS, and Claude; registry discovery and the other built-in descriptors
do not widen that reviewed set. Codex remains withheld because `mode=read-only`
blocks writes but does not permission-route passive reads, while the standard
sandbox leaves credential homes readable. Claude is gated before the first
prompt. The withheld goose path uses `approve` rather than its default
`auto` (auto-approve);
...
> `pi-acp` binary (not `pi acp`). Only Kiro CLI, KAS, and Claude are
