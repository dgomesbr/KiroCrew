...
export function useAgentSync() {
  ...
  const { data: defaultAgentData } = useQuery({
    queryKey: ['default-agent'],
    queryFn: () => api.defaultAgent().then(d => d.default_agent || ''),
    // Finite staleTime opts into refetch-on-focus (the shared client pins
    // staleTime: Infinity precisely so queries opt in this way), so a
    // long-lived Worlds window or popout self-heals when refocused after the
    // default changed in another window.
    staleTime: 30_000,
  })
  ...
  const slotAgents = useMemo<AgentSource[]>(() => slots.map(sl => ({
    ...
    label: sl.agent || defaultAgent || 'default', kind: 'slot' as const,
